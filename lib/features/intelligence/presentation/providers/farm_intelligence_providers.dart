...
final farmVisitorEventsProvider =
  ...
  try {
    ...
    return Stream.value(<VisitorEventModel>[]);
  }
});
...
final allCasesWithLocationProvider =
  ...
  try {
    ...
    return Stream.value(<HealthCaseWithLocation>[]);
  }
});
...
final activeOutbreakClustersProvider =
  ...
  try {
    ...
    return Stream.value(<OutbreakClusterModel>[]);
  }
});
...
final farmHealthIntelligenceProvider =
  ...
  final visitorEvents = ref.watch(farmVisitorEventsProvider(farmId)).valueOrNull ?? <VisitorEventModel>[];
  final nearbyCandidates = ref.watch(allCasesWithLocationProvider).valueOrNull ?? <HealthCaseWithLocation>[];
  final activeClusters = ref.watch(activeOutbreakClustersProvider).valueOrNull ?? <OutbreakClusterModel>[];
