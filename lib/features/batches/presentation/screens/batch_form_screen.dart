...
class _BatchFormScreenState extends State<BatchFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      final effectiveHatch = _hatchDate ?? DateTime.now();
      final effectivePlacement = _placementDate ?? DateTime.now();
      final effectiveFlockType = _selectedFlockType ?? 'Broiler';
      ...
      final batch = await BatchService.createBatch(
        ...
        widthFt: _breadthFt,
        sizeUnit: _sizeUnit,
        hatchDate: effectiveHatch,
        placementDate: effectivePlacement,
        maleCount: _maleCount,
        femaleCount: _femaleCount,
        breedOrFlockType: effectiveFlockType,
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Unable to save batch: $e'), backgroundColor: AppColors.danger));
    } finally {
