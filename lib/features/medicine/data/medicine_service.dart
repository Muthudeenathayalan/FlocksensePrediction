...
class MedicineService {
  ...
  static Stream<List<MedicineRecordModel>> watchMedicineRecords(
    ...
    try {
      ...
      return _medicineRef(uid, farmId, batchId).snapshots().map((
        snapshot,
      ) {
  ...
  static Future<void> deleteMedicineRecord(
    ...
    try {
      final uid = _auth.currentUser?.uid ?? 'farmer_demo_user';
      await _medicineRef(uid, farmId, batchId).doc(recordId).delete();
    } catch (e) {
      debugPrint('MedicineService.deleteMedicineRecord note: $e');
    }
  }
}
