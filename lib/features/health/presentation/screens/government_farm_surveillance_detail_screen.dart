...
class GovernmentFarmSurveillanceDetailScreen extends ConsumerWidget {
          ...
          WebPageHeader(
            ...
            actions: [
              Container(
                ...
                child: const Row(
                  children: [
                    Icon(Icons.policy_outlined, size: 16, color: AppColors.info),
                    SizedBox(width: 6),
                    Text('Authorized Government Surveillance (Read-Only)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.info)),
                  ],
                ),
              ),
  ...
  Widget _buildBreadcrumbItem(BuildContext context, String text, bool isLink) {
    return Text(
      ...
      style: TextStyle(
        fontSize: 12,
        fontWeight: isLink ? FontWeight.w500 : FontWeight.w700,
        color: isLink ? AppColors.primary : AppColors.slate900,
      ),
    );
  }
